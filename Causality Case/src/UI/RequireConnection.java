...
public class RequireConnection extends JFrame implements ActionListener{
	private RequireConnection rc;
	...
	public RequireConnection(DrawJpanel djp,JPanel jp,Require require,List<Status> staList) {
		rc = this;
		this.statusList = new ArrayList<Status>(staList);
		this.require = require;
		if(require != null) {
			this.selList = new ArrayList<Status>(require.getStatusList());
		}
		else {
		...
		this.drawJpanel = djp;
		this.jpanel = jp;
		createRequireConnection();
	}

	private void createRequireConnection() {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource() == addButton) {
			new AddStatusList(rc,statusList,selList).setVisible(true);
		}
		...
		if(arg0.getSource() == OKButton) {
			if(selList.size() < 2) {
				...
				if(require != null)
					ch.deleteRequire(drawJpanel.getRequireList(), drawJpanel.getPointList(), drawJpanel.getRequireList().indexOf(require));
				ch.createRequire(drawJpanel,selList);
				jpanel.repaint();
				dispose();
			}
		}
