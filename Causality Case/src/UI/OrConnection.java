...
public class OrConnection extends JFrame implements ActionListener{
	...
	private OrConnection oc = this;
	private JPanel jpanel;
	private DrawJpanel drawJpanel;
	private Or or;
	
	public OrConnection(DrawJpanel djp,JPanel jp,Or or,List<Status> staList) {
		// TODO Auto-generated constructor stub
		inputList = new ArrayList<Status>();
		this.statusList = new ArrayList(staList);
		this.or = or;
		if(or != null) {
			this.inputList = new ArrayList<Status>(or.getStartList());
			this.outputStatus = or.getEnd();
	...
	private void createOrConnection() {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource() == addButton1) {
			new AddStatusList(oc,statusList,inputList).setVisible(true);
		}
		if(arg0.getSource() == addButton2) {
			if(outputStatus == null) {
				 new AddStatus(oc,statusList).setVisible(true);
			}
			else {
				String message = "只允许存在一个输出状态！";
				JOptionPane.showMessageDialog(oc, message);
			}
		}
		...
		if(arg0.getSource() == OKButton) {
			...
			if(or != null)
				ch.deleteOr(drawJpanel.getOrList(), drawJpanel.getPointList(), drawJpanel.getOrList().indexOf(or));
			ch.createOr(drawJpanel,inputList,outputStatus);
			jpanel.repaint();
			dispose();
		}
